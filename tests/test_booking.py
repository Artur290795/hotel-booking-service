...
class TestCreateBooking:
    ...
    def test_create_booking_with_conflicting_data(
        self,
        client,
        add_room_to_db,
        booking_factory,
        ...
        booking_data1 = booking_factory(
            room_id=room.id, start_date="2026-01-01", finish_date="2026-01-02"
        )
        booking_data2 = booking_factory(
            room_id=room.id, start_date="2026-01-03", finish_date="2026-01-04"
        )
        client.post("/bookings/", data=booking_data1)
        client.post("/bookings/", data=booking_data2)
        booking_data3 = booking_factory(
            room_id=room.id, start_date=start_date, finish_date=finish_date
        )
...
class TestGetBooking:
    ...
    def test_get_booking_by_room_id(self, client, add_room_to_db, booking_factory):
        ...
        booking_data1.update(client.post("/bookings/", data=booking_data1).json())
        booking_data2.update(client.post("/bookings/", data=booking_data2).json())
        booking_data3.update(client.post("/bookings/", data=booking_data3).json())
    ...
    @pytest.mark.django_db
    def test_get_booking_without_room_id(self, client):
